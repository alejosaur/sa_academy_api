"main";let ApolloServer,gql;_f3e‍.w("apollo-server",[["ApolloServer",["ApolloServer"],function(v){ApolloServer=v}],["gql",["gql"],function(v){gql=v}]]);let CoursesAPI;_f3e‍.w("./datasource",[["CoursesAPI",["CoursesAPI"],function(v){CoursesAPI=v}]]);
...
const resolvers = {
    Query: {
        course: (root, { idNumber }, { dataSources }) => dataSources.ResidentAPI.getAResident(idNumber),
        courses: (root, args, { dataSources }) => dataSources.ResidentAPI.getAllResidents(),
    },
    Mutation: {
...
server.listen().then(({ url }) => {
    _f3e‍.g.console.log(`🚀 Server ready at ${url}`);
});

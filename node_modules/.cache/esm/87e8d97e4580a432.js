"main";let ApolloServer,gql;_458‍.w("apollo-server",[["ApolloServer",["ApolloServer"],function(v){ApolloServer=v}],["gql",["gql"],function(v){gql=v}]]);let ResidentAPI,ReportsAPI,TransactionAPI;_458‍.w("./datasource",[["ResidentAPI",["ResidentAPI"],function(v){ResidentAPI=v}],["ReportsAPI",["ReportsAPI"],function(v){ReportsAPI=v}],["TransactionAPI",["TransactionAPI"],function(v){TransactionAPI=v}]]);yield;
...
const typeDefs = gql`
    ...
    type Report {
        id: ID!
        report_type(report_type_id: report_type_id): [Report_type]
        created_at: String
        IP: String
    }
...
const resolvers = {
    Query: {
        ...
        reports: (root, args, { dataSources }) => dataSources.ReportsAPI.getAllReports(idNumber),
        report_types: (root, args, { dataSources }) => dataSources.ReportsAPI.getAType(),
    ...
    Report: {
        report_type: (root, { report_type_id }, { dataSources }) =>  dataSources.ReportsAPI.getAType(report_type_id),
    }
};
...
server.listen().then(({ url }) => {
    _458‍.g.console.log(`🚀 Server ready at ${url}`);
});
